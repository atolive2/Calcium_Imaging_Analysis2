        ...
        if isfield(allData{1,t}, 'correlated_ROIs_dff0_MS_common_AROI')
            if isfield(allData{1,t}, 'correlated_ROIs_dff0_M_common_AROI')
                if isfield(allData{1,t}, 'correlated_ROIs_dff0_V_common_AROI')
                    ...
                    scatter(allData{1,t}.ROIcenters(allData{1,t}.highcorr_M_V, 1), allData{1,t}.ROIcenters(allData{1,t}.highcorr_M_V, 2), 150, 'o', 'filled', 'MarkerEdgeColor', [0.5 0 0.5], 'MarkerFaceColor', [0.5 0 0.5])
                    % all M and MS highcorr ROIs as green filled circles
                    scatter(allData{1,t}.ROIcenters(allData{1,t}.highcorr_M_MS, 1), allData{1,t}.ROIcenters(allData{1,t}.highcorr_M_MS, 2), 150, 'o', 'filled', 'MarkerEdgeColor', [0.4 0.5 0.2], 'MarkerFaceColor', [0.4 0.5 0.2])
                    % all V and MS highcorr ROIs as orange filled circles
                    scatter(allData{1,t}.ROIcenters(allData{1,t}.highcorr_M_V, 1), allData{1,t}.ROIcenters(allData{1,t}.highcorr_M_V, 2), 150, 'o', 'filled', 'MarkerEdgeColor', [1 0.6 0.2], 'MarkerFaceColor', [1 0.6 0.2])
...
%% 46 to 49 basic comparison - better size labesl
...
for i = 3:27
    ...
    fig_filename = sprintf('st 46 vs 49 ECDF of %s large labels', labels{i-2})
...
allRespROIs_PCAsub = allRespROIs(:, [7:9, 15, 16, 18:20, 26, 27]);
labels_PCAsub = labels([5:7, 13, 14, 16:18, 24, 25])
...
ylabel('percent')
set(gca, 'fontsize', 30)
saveas(gcf, 'PCA var explained all data large labels', 'png')
...
ylabel('percent')
set(gca, 'fontsize', 30)
saveas(gcf, 'PCA var explained st46 large labels', 'png')
...
ylabel('percent')
set(gca, 'fontsize', 30)
saveas(gcf, 'PCA var explained st49 large labels', 'png')

% plot MSEnh peak against PC1 value for all cells
figure;
scatter(score_all(:,1), allRespROIs_PCAsub(:,4))
...
ylim([-1 10])
set(gca, 'fontsize', 30)
saveas(gcf, 'MSEnh peak vs PC1 allRespROIs large labels', 'png')

figure;
scatter(score_46(:,1), allRespROIs_PCAsub(st46,4))
...
ylim([-1 10])
set(gca, 'fontsize', 30)
saveas(gcf, 'MSEnh peak vs PC1 st46 only large labels', 'png')

figure;
scatter(score_49(:,1), allRespROIs_PCAsub(st49, 4))
...
ylim([-1 10])
set(gca, 'fontsize', 30)
saveas(gcf, 'MSEnh peak vs PC1 st49 only large labels', 'png')